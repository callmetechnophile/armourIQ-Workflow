...
def speech_to_text(audio_bytes: bytes, filename: str = "audio.wav") -> str:
    ...
    Uses Groq as primary and Sarvam AI as secondary.
    """
    if not audio_bytes:
        return ""
    ...
    if groq_key and not groq_key.startswith("gsk_placeholder"):
        try:
            logger.info("Attempting Groq Whisper STT (Primary)")
    ...
    # 2. Try Sarvam AI (Secondary)
    sarvam_key = os.environ.get("SARVAM_API_KEY")
    if sarvam_key and not sarvam_key.startswith("sarvam_placeholder"):
        try:
            logger.info("Attempting Sarvam AI STT (Secondary)")
